...
var Memory = exports.Memory = function Memory() {
    ...
    function _get(uri) {
        var result = {};
        result[uri] = _descriptions[uri];
        return result
    }

    function _update(uri, description) {
        /*
         * Handle the case when descripted is keyed by the subject at the first level.
         * Note that the subject can be a valid property name (e.g. the rdfs:label of rdfs:label).
         * We therefore need to check whether the properties value is an array, in which case
         * it is a property key.
         */
        if (description.hasOwnProperty(uri) && !util.isArray(description[uri])) {
            description = description[uri];
        }

        _descriptions[uri] = _descriptions[uri] || {};

        Object.keys(description).forEach(function (predicate) {
            _descriptions[uri][predicate] = _descriptions[uri][predicate] || [];
            description[predicate].forEach(function (object) {
                _descriptions[uri][predicate].push(object);
            });
        });
    }
