/*
 * Connect middleware interface
 */

var config = require(require('path').join(__dirname, '..', 'config'));

exports = module.exports = function (options) {
    var mimeTypes = Object.keys(config.typeMapping);

    return function (request, response, next) {
        var accept = request.headers['accept'];
        var match = Mimeparse.bestMatch(mimeTypes, accept) || config.defaults.type;
        response.mimeType = match;
        response.type = config.typeMapping[match];
        next();
    }
};
...
var Mimeparse = (function() {
    ...
    var publicMethods = {
        ...
        fitnessAndQualityParsed : function(mimeType, parsedRanges) {
            ...
            if (parsedRanges.length < 1 && targetType == "*" && targetSubtype == "*") {
                bestFitness = 1;
                bestFitQ = 1.0;
            }
