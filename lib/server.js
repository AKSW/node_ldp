    ...
    try {
        ...
        parser.on('statement', function (statement) {
            if (statement.subject.type == 'uri') {
                ...
                rdf.mixinStatement(blankNodes, statement);
            }
        });
        parser.on('end', function () {
            var store  = new Memory();
            Object.keys(descriptions).forEach(function (uri) {
                store.create(uri, descriptions);
                // FIXME: only add blank nodes to a resource that are
                // actually (transitively) referenced
                store.update(uri, blankNodes);
            });
